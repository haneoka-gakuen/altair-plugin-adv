...
export const validateStoryProject = (value: unknown): StoryProjectValidationResult => {
  ...
  if (value.plugins !== undefined) {
    if (!Array.isArray(value.plugins)) {
      add("error", "project.plugins", "$.plugins", "plugins must be an array");
    } else {
      const pluginIds = new Set<string>();
      for (const [index, plugin] of value.plugins.entries()) {
        const path = `$.plugins[${index}]`;
        if (!record(plugin)) {
          add("error", "plugin.type", path, "Plugin state must be an object");
          continue;
        }
        const { enabled: _enabled, ...canonical } = plugin;
        try {
