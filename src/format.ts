...
import { STORY_PROJECT_VERSION, type JsonObject } from "@haneoka/altair/model";
import type { AltairFormatContribution, AltairSourceFile } from "@haneoka/altair/plugins";
import { importStoryProjectJson, serializeStoryProjectJson } from "./project-json.js";
...
const abort = (signal: AbortSignal): void => {
  if (signal.aborted) {
    throw signal.reason ?? new DOMException("ADV format operation aborted", "AbortError");
  }
};
...
const booleanOption = (options: JsonObject | undefined, key: string): boolean | undefined => {
  const value = options?.[key];
  return typeof value === "boolean" ? value : undefined;
};

const importOptions = (options: JsonObject | undefined): ImportAdvStoryOptions => ({
  ...(stringOption(options, "title") === undefined ? {} : { title: stringOption(options, "title")! }),
  ...(stringOption(options, "sceneId") === undefined ? {} : { sceneId: stringOption(options, "sceneId")! }),
  ...(stringOption(options, "sceneName") === undefined ? {} : { sceneName: stringOption(options, "sceneName")! }),
  ...(stringOption(options, "releaseServer") === undefined
    ? {}
    : { releaseServer: stringOption(options, "releaseServer")! }),
  ...(record(options?.provenance) ? { provenance: options.provenance } : {}),
});

const serializeOptions = (options: JsonObject | undefined): SerializeAdvStoryOptions => ({
  ...(stringOption(options, "sceneId") === undefined ? {} : { sceneId: stringOption(options, "sceneId")! }),
  ...(booleanOption(options, "pretty") === undefined ? {} : { pretty: booleanOption(options, "pretty")! }),
  ...(booleanOption(options, "preserveSourceContainer") === undefined
    ? {}
    : {
        preserveSourceContainer: booleanOption(options, "preserveSourceContainer")!,
      }),
});
...
const sniffProjectJson = (file: AltairSourceFile): number => {
  try {
    const parsed = JSON.parse(decoder.decode(file.bytes)) as unknown;
    if (!record(parsed)) return 0;
    return parsed.version === STORY_PROJECT_VERSION &&
...
export const PROJECT_JSON_FORMAT: AltairFormatContribution = Object.freeze<AltairFormatContribution>({
  id: "project-json",
  name: "Altair project JSON",
  extensions: [".story.json", ".json"],
  mediaTypes: ["application/json", "application/vnd.haneoka.story-project+json"],
  sniff(request) {
  ...
  import(request) {
    abort(request.signal);
    const imported = importStoryProjectJson(entryFile(request.files, request.entryPath).bytes);
    abort(request.signal);
    return imported;
  },
  export(request) {
...
export const ADV_JSON_FORMAT: AltairFormatContribution = Object.freeze<AltairFormatContribution>({
  id: "adv-json",
  name: "ADV JSON",
  ...
  export(request) {
    abort(request.signal);
    const source = serializeAdvStoryJson(request.project, serializeOptions(request.options));
    abort(request.signal);
    return {
      artifacts: [
        {
