...
import { compileStoryProject } from "./compiler.js";
import {
...
import { isJsonObject, jsonClone, jsonInput, stringifyStoryJson, type StoryImportResult } from "./json.js";
...
const EPISODE_ROW_FIELDS = [
  ...
  ["_hiddenParameter1", "hiddenParameter1"],
] as const;
...
const normalizeAdvFieldAliases = (value: JsonObject): JsonObject => {
  const fields = cloneStoryValue(value);
  if (!own(fields, "hiddenParameter1")) {
    if (own(fields, "HiddenParameter1")) {
      fields.hiddenParameter1 = cloneStoryValue(fields.HiddenParameter1!);
    } else if (own(fields, "_hiddenParameter1")) {
      fields.hiddenParameter1 = cloneStoryValue(fields._hiddenParameter1!);
    }
  }
  delete fields.HiddenParameter1;
  delete fields._hiddenParameter1;
  return fields;
};

const splitAdvCodecFields = (value: JsonObject): { fields: JsonObject; codecFields: JsonObject } => {
  const fields = normalizeAdvFieldAliases(value);
